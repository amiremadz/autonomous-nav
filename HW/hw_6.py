...
class UserCode:
    ...
    def calculatePredictMeasurementJacobian(self, x, marker_position_world, marker_yaw_world):
        ...
        dRotationT_dYaw = np.array([
                            [-s_yaw, c_yaw],
                            [-c_yaw, -s_yaw]
                            ])

        rotationT = self.rotation(yaw).T
        
        dXdY = -np.eye(2)
        H[0:2, 0:2] = np.dot(rotationT, dXdY)
        H[0:2, 2] = np.dot(dRotationT_dYaw, marker_position_world - x[0:2])
        H[2, 2] = -1
